...
public:
    KeywordExtraction()
    {
        if (checkFile("WordKey.key"))
        {
            ...
            /*Tạo không gian từ key*/
            CryptoPP::ArraySink copykey(wordKey, sizeof(wordKey));
            /*Copy data từ FileKey.key tới key */
            fs.Detach(new Redirector(copykey));
            fs.Pump(sizeof(wordKey)); // Pump 16 bytes đầu
        }
        else
        {
            AutoSeededRandomPool prng;
    ...
    void GetFileKeyWord(string filename, string ToKen, string Pathfile)
    {
        ...
        // Filename cũng được tính là một từ khóa
    ...
    // Băm fileKeyword và wordKey thành trapdoor
    void genTrapdoor()
    {
        for (int i = 0; i < fileKeyword.size(); i++)
            keyWordTrapdoor.push_back(HMACSHA3_256(wordKey, fileKeyword[i]));
    }
    // Hàm thêm trapdoor (vào cột keyword tương ứng với token) vào bảng QUERY_KEYWORD_TABLE
    void addTrapdoor()
    {
...
int main()
{
    ...
    cout<<"\n";
    cout<<"\t----------MS-SDC----------\n";
    cout<<"1. Uploadfile [filename] [fileKeyWord]"<<endl;
    cout<<"\tUpload filename to cloud with keyword lists stored in fileKeyWord.\n";
    cout<<"2. Seach [keyword]"<<endl;
    cout<<"\tSearch for files with the keyword \"keyword\""<<endl;
    cout<<"3. Downloadfile [number]"<<endl;
    cout<<"\tDownload files whose index in the searched files is \"number\""<<endl;
